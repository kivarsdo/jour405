...
Spotify_songs |> 
  ggplot() + 
  geom_histogram(aes(x = danceability), binwidth = 0.5)
...
The distribution in the danceability scores more closely resembled a skewed left distribution rather than a normal distribution, in my opinion. The mean tells us that most songs, on average, have about a 60 percent danceability score. On the other hand, the distribution being skewed left tells us that the songs that have little to no danceability such as "water sound natural white noise" are the outliers that are skewing the data.
